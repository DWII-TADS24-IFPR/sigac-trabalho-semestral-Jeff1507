<x-app-layout>
    ...
    <section class="flex items-center justify-center">
        <form action="{{ route('documento.update', $documento->id) }}" method="POST" enctype="multipart/form-data" class="w-full sm:w-96 shadow-sm sm:border border-[#49454F] flex flex-col gap-8 px-6 py-4">
            ...
            <x-title>
                Editar Solicitação Horas Complementares
            </x-title>
            <div class="w-full flex flex-col space-y-1.5">
                <x-input-label for="descricao" :value="__('Descrição do Documento')" />
                <x-text-input id="descricao" class="block mt-1 w-full" type="text" name="descricao" value="{{ old('descricao', $documento->descricao) }}" required autofocus />
                <x-input-error :messages="$errors->get('descricao')" class="mt-2" />
            </div>
            <div class="w-full flex flex-col space-y-1.5">
                <x-input-label for="horas_in" :value="__('Horas Solicitadas')" />
                <x-text-input id="horas_in" class="block mt-1 w-full" type="number" name="horas_in" value="{{ old('horas_in', $documento->horas_in) }}" required autofocus />
                <x-input-error :messages="$errors->get('horas_in')" class="mt-2" />
            </div>
            <div class="w-full flex flex-col space-y-1.5">
                <x-input-label for="categoria_id" :value="__('Categoria do documento')" />
                <x-select :options="$categorias" :selected="old('categoria_id', $documento->categoria_id)" name="categoria_id" label="Selecione uma categoria"/>
            ...
            <div class="flex justify-start space-y-1.5">
                <x-link href="{{ asset('storage/' . $documento->url) }}" target="_blank" rel="noopener noreferrer">
